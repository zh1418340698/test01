...
<%@ page import="com.zenghao.crm.settings.domain.DicValue" %>
<%@ page import="java.util.Map" %>
<%@ page import="com.zenghao.crm.workbench.domain.Tran" %>
...
<script >
		...
		$.ajax({
			...
			success:function (data) {
				...
				if (data.success){
					...
					//改变阶段成功后
					//(1)需要在详细信息页上局部刷新 阶段、可能性、修改人、修改时间
					$("#stage").html(data.t.stage);
					$("#possibility").html(data.t.possibility);
					$("#editBy").html(data.t.editBy);
					$("#editTime").html("&nbsp&nbsp"+data.t.editTime);

					//改变阶段成功后
					//(2)将所有的图标重新判断，重新赋予样式及颜色
					changeIcon(stage,i);
				}else {
					alert("改变阶段失败");
				}
	...
	function changeIcon(stage,index1) {
		...
		//如果当前的可能性为0，前7个一定是黑圈，后两个，一个是黑叉，一个红叉
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			int index = 0;
			for (int i = 0;i<dvList.size();i++){
				if (currentStage.equals(dvList.get(i).getValue())){
					index = i;
					break;
				}
			}

			//如果当前阶段可能性为0，则前面7个都为黑圈，后面两个 一个黑×一个红×
		...
		<span id="<%=i%>" onclick="changeStage('<%=dvList.get(i).getValue()%>','<%=i%>')" class="glyphicon glyphicon-record mystage" data-toggle="popover"
			  data-placement="bottom" data-content="<%=dvList.get(i).getText()%>"
		...
		<span id="<%=i%>" onclick="changeStage('<%=dvList.get(i).getValue()%>','<%=i%>')" class="glyphicon glyphicon-remove mystage" data-toggle="popover"
			  data-placement="bottom" data-content="<%=dvList.get(i).getText()%>"
		...
		<span onclick="changeStage('<%=dvList.get(i).getValue()%>','<%=i%>')" id="<%=i%>" class="glyphicon glyphicon-remove mystage" data-toggle="popover"
			  data-placement="bottom" data-content="<%=dvList.get(i).getText()%>"
			  style="color: #000000;"> </span>
		-----------
		...
		<span id="<%=i%>" onclick="changeStage('<%=dvList.get(i).getValue()%>','<%=i%>')" class="glyphicon glyphicon-ok-circle mystage" data-toggle="popover"
			  data-placement="bottom" data-content="<%=dvList.get(i).getText()%>"
		...
		<span id="<%=i%>" onclick="changeStage('<%=dvList.get(i).getValue()%>','<%=i%>')" class="glyphicon glyphicon-map-marker mystage" data-toggle="popover"
			  data-placement="bottom" data-content="<%=dvList.get(i).getText()%>"
			  style="color: #90F790;"> </span>
		-----------
